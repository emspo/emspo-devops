...
const PORT = 8888;
const VALID_TOKEN = 'DOCKER_HUB_TOKEN';
const VALID_USER = 'DOCKER_HUB_USER';
...
function gitPullForDevelopment(type, callback) {
	...
	exec(`git -C ${folder} pull origin --ff-only`, (err, stdout, stderr) => {
		...
		if (callback) {
			...
			console.log(timestamp(),`[Git Pull] Success in ${folder}:`, stdout.trim())
		}
	})
}
...
function dockerDeploy(type, tag, callback) {
	...
	const image = `emspo/${imagePrefix}:${tag}`
	const composeFile = `mspo/be/Docker/${tag}-docker-compose.yml`  // updated path here
	const loginCommand = `echo ${VALID_TOKEN} | docker login -u ${VALID_USER} --password-stdin`
	let projectName;
	const commands = [
		loginCommand,
		`docker pull ${image}`,
...
const server = http.createServer((req, res) => {
	...
	req.on('end', () => {
		try {
			...
			console.log("Request made for:", type, tag);
			if (tag === 'development') {
				if (type === 'be') {
